...
std::optional<GGUFMetadata> read_gguf_metadata(const fs::path& model_path) {
    ...
    if (filename.find("q4_k_m") != std::string::npos) meta.quantization = "Q4_K_M";
    else if (filename.find("q4_k_s") != std::string::npos) meta.quantization = "Q4_K_S";
    else if (filename.find("q5_k_m") != std::string::npos) meta.quantization = "Q5_K_M";
    else if (filename.find("q5_k_s") != std::string::npos) meta.quantization = "Q5_K_S";
    else if (filename.find("q8_0") != std::string::npos) meta.quantization = "Q8_0";
    else if (filename.find("f16") != std::string::npos) meta.quantization = "F16";
    else meta.quantization = "Unknown";
    
    if (filename.find("llama") != std::string::npos) meta.architecture = "llama";
    else if (filename.find("mistral") != std::string::npos) meta.architecture = "mistral";
    else if (filename.find("phi") != std::string::npos) meta.architecture = "phi";
    else if (filename.find("qwen") != std::string::npos) meta.architecture = "qwen";
    else meta.architecture = "unknown";
...
public:
    ...
    bool load(const LLMConfig& config) override {
        ...
        ctx_params.n_threads = config.n_threads > 0 ? config.n_threads : 
    ...
    std::string generate(std::string_view prompt, const GenerationParams& params) override {
        ...
        while (n_generated < params.max_tokens) {
            // Sample next token
            llama_token new_token = sample_token(params, n_cur);
...
private:
    llama_token sample_token(const GenerationParams& params, int pos) {
        ...
        if (params.temperature <= 0) {
            // Greedy
            return llama_sample_token_greedy(ctx_, &candidates_p);
        }
        
        // Apply repetition penalty
        // llama_sample_repetition_penalties(...) - simplified for now
        
        // Temperature
        llama_sample_temp(ctx_, &candidates_p, params.temperature);
        
        // Top-k
        llama_sample_top_k(ctx_, &candidates_p, params.top_k, 1);
