...
namespace vdb
{
    namespace index
    {
                                                   ...
                                                   const std::string &field) const
        {
            ...
            if (field == "type")
                return std::to_string(static_cast<int>(metadata.type));
            if (field == "asset")
                return metadata.asset;
            if (field == "bias")
                return metadata.bias;
                                                      ...
                                                      const std::string &value) const
        {
            ...
            if (posting_it == it->second.postings.end())
                return {};
        ...
        {
            auto it = indices_.find(field);
            if (it == indices_.end() || !it->second.is_numeric)
                return {};

            std::set<VectorId> result;

            auto lower = min_inclusive ? it->second.numeric_index.lower_bound(min_val) : it->second.numeric_index.upper_bound(min_val);

            auto upper = max_inclusive ? it->second.numeric_index.upper_bound(max_val) : it->second.numeric_index.lower_bound(max_val);
