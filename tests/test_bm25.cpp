...
void test_bm25_search() {
    ...
    assert(found_doc1);
    assert(found_doc3);
    std::cout << "  ✓ Expected documents found\n";
    
    // Check that results are sorted by score (descending)
    for (size_t i = 1; i < results->size(); ++i) {
        assert((*results)[i-1].score >= (*results)[i].score);
    }
    std::cout << "  ✓ Results properly sorted\n";
...
void test_bm25_stemming() {
    ...
    std::cout << "  ✓ Stemming working\n";
    
    std::cout << "✓ Stemming test passed\n\n";
}
...
void test_bm25_empty_query() {
    ...
    std::cout << "  ✓ Empty query handled correctly\n";
    
    // Query with only stop words
    results = engine.search("the and or", 10, 0.0f);
    assert(!results);  // Should fail (no valid terms)
    std::cout << "  ✓ Stop-word-only query handled correctly\n";
    
    std::cout << "✓ Empty query test passed\n\n";
}
...
void test_bm25_case_sensitivity() {
    ...
    std::cout << "  ✓ Case insensitive search working\n";
