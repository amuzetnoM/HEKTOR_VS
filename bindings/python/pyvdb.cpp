...
#include <pybind11/stl/filesystem.h>  // For std::filesystem::path support
...
PYBIND11_MODULE(pyvdb, m)
{
    ...
    py::class_<VectorDatabase>(m, "VectorDatabase")
        ...
        // Text operations
        .def("add_text", [](VectorDatabase &self, const std::string &text, const Metadata &meta)
             {
            auto result = self.add_text(text, meta);
            if (!result) {
                throw std::runtime_error(result.error().message);
            }
            return *result; }, py::arg("text"), py::arg("metadata"))

        .def("add_text", [](VectorDatabase &self, const std::string &text, DocumentType type, const std::string &date)
             {
            auto result = self.add_text(text, type, date);
            if (!result) {
                throw std::runtime_error(result.error().message);
            }
            return *result; }, py::arg("text"), py::arg("type"), py::arg("date"))

        .def("query_text", [](VectorDatabase &self, const std::string &query, const QueryOptions &options)
             {
            auto result = self.query_text(query, options);
            if (!result) {
                throw std::runtime_error(result.error().message);
            }
            return *result; }, py::arg("query"), py::arg("options") = QueryOptions())
        ...
        .def("search", [](VectorDatabase &self, const std::string &query, size_t k)
            ...
            return *result; }, py::arg("query"), py::arg("k") = 10)

        // Image operations
        .def("add_image", [](VectorDatabase &self, const std::string &path, const Metadata &meta)
             {
            auto result = self.add_image(path, meta);
            if (!result) {
                throw std::runtime_error(result.error().message);
            }
            return *result; }, py::arg("path"), py::arg("metadata"))
        ...
        .def("add_image", [](VectorDatabase &self, const std::string &path)
            ...
            return *result; }, py::arg("path"))

        .def("query_image", [](VectorDatabase &self, const std::string &path, const QueryOptions &options)
             {
            auto result = self.query_image(path, options);
            if (!result) {
                throw std::runtime_error(result.error().message);
            }
            return *result; }, py::arg("path"), py::arg("options") = QueryOptions())

        // Vector operations (with numpy support)
        .def("add_vector", [](VectorDatabase &self, py::array_t<float> vec, const Metadata &meta)
             {
            auto result = self.add_vector(numpy_to_view(vec), meta);
            if (!result) {
                throw std::runtime_error(result.error().message);
            }
            return *result; }, py::arg("vector"), py::arg("metadata"))

        .def("query_vector", [](VectorDatabase &self, py::array_t<float> vec, const QueryOptions &options)
             {
            auto result = self.query_vector(numpy_to_view(vec), options);
            if (!result) {
                throw std::runtime_error(result.error().message);
            }
            return *result; }, py::arg("vector"), py::arg("options") = QueryOptions())
        ...
        .def("get_vector", [](VectorDatabase &self, VectorId id) -> py::object
            ...
            return vector_to_numpy(*vec); }, py::arg("id"))
    ...
    m.def("create_gold_standard_db", [](const std::string &path)
        ...
        return std::move(*result); }, py::arg("path"), "Create a new database optimized for Gold Standard");
    ...
    m.def("open_database", [](const std::string &path)
        ...
        return std::move(*result); }, py::arg("path"), "Open an existing database");
