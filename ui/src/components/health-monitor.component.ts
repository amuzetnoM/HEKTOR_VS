import { Component, ElementRef, ViewChild, signal, OnDestroy, AfterViewInit } from '@angular/core';
      ...
      <div class="max-w-7xl mx-auto space-y-10">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          <div class="bg-white/[0.02] border border-white/5 p-6 rounded-2xl hover:bg-white/[0.04] transition-colors group">
             ...
             <div class="mt-2 text-xs text-emerald-400 font-mono">All services healthy</div>
          </div>
           <div class="bg-white/[0.02] border border-white/5 p-6 rounded-2xl hover:bg-white/[0.04] transition-colors">
             <div class="flex justify-between items-start mb-4">
               <div class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Uptime</div>
               <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-zinc-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
             </div>
             <div class="text-3xl font-mono text-white tracking-tighter">99.99<span class="text-lg text-zinc-600">%</span></div>
          </div>
           <div class="bg-white/[0.02] border border-white/5 p-6 rounded-2xl hover:bg-white/[0.04] transition-colors">
             <div class="flex justify-between items-start mb-4">
               <div class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Active Nodes</div>
             ...
             <div class="text-3xl font-mono text-white tracking-tighter">3 <span class="text-lg text-zinc-600">/ 3</span></div>
          </div>
           <div class="bg-white/[0.02] border border-white/5 p-6 rounded-2xl hover:bg-white/[0.04] transition-colors">
             <div class="flex justify-between items-start mb-4">
               <div class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Error Rate</div>
               <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-zinc-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
             </div>
             <div class="text-3xl font-mono text-indigo-400 tracking-tighter">0.01<span class="text-lg text-zinc-600">%</span></div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div class="bg-black/20 border border-white/5 rounded-2xl p-6 h-[300px] md:h-[400px] flex flex-col relative overflow-hidden ring-1 ring-white/5">
            <div class="flex justify-between items-center mb-6 z-10 relative">
              ...
              <div class="text-xs font-mono text-zinc-500">AVG: 24ms</div>
            </div>
          ...
          <div class="bg-black/20 border border-white/5 rounded-2xl p-6 h-[300px] md:h-[400px] flex flex-col relative overflow-hidden ring-1 ring-white/5">
             <div class="flex justify-between items-center mb-6 z-10 relative">
               ...
               <div class="text-xs font-mono text-zinc-500">PEAK: 250/s</div>
             </div>
...
export class HealthMonitorComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    for (let i = 0; i < this.maxPoints; i++) {
      this.latencyData.push({ time: now - (this.maxPoints - i) * 1000, value: 20 + Math.random() * 10 });
      this.tpsData.push({ time: now - (this.maxPoints - i) * 1000, value: 100 + Math.random() * 50 });
    }
  ...
  private updateData() {
    ...
    this.latencyData.push({ time: now, value: 20 + Math.random() * 8 + spike });
    this.tpsData.push({ time: now, value: 150 + Math.random() * 80 });
  ...
  private drawChart(container: HTMLElement, data: any[], color: string) {
    ...
    // Tighter margins for cleaner look
    ...
    const y = d3.scaleLinear()
      .domain([0, d3.max(data, (d: any) => d.value) * 1.3])
      .range([height - margin.bottom, margin.top]);

    const line = d3.line()
      .x((d: any) => x(d.time))
      .y((d: any) => y(d.value))
      .curve(d3.curveBasis); // Smoother curves
    ...
    // No Axis lines, just pure data visualization
  }
}